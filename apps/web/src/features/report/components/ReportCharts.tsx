...
/** Followers vs views, log/log. */
export function FollowersViewsScatter({ rows, colors }: ChartProps) {
  const option = useMemo<EChartsOption>(() => {
    return {
      tooltip: {
        formatter: (p) => {
          const point = p as unknown as { name: string; value: [number, number] };
          return `${point.name}<br>Followers: ${fmt(point.value[0])}<br>Views: ${fmt(point.value[1])}`;
        },
      },
      ...
      series: [
        {
          type: 'scatter',
          symbolSize: 12,
          // Colour is set per point rather than via an itemStyle callback, so the
          // category mapping stays type-checked.
          // A log axis cannot plot 0, so both coordinates are floored at 1.
          data: rows.map((r) => ({
            value: [Math.max(r.followers, 1), Math.max(r.views, 1)] as [number, number],
            name: r.username,
            itemStyle: { color: colors.colorOf(r.category) },
          })),
        },
  ...
  return <EChart option={option} height={340} ariaLabel="Followers vs Views" />;
}
